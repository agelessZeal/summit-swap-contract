...
methods {
	...
	// SUMMIT token
	SUMMIT() returns (address) envfree

	// Rewarder
	onSummitReward(uint256, address, address, uint256, uint256) => NONDET
...
rule noChangeToOtherPool(uint256 pid, uint256 otherPid) {
	...
	assert(_otherAccSushiPerShare == otherAccSushiPerShare_, "accSummitPerShare changed");
...
rule preserveTotalAssetOfUser(method f, uint256 pid, address user,
	...
	require SUMMIT() != lpToken(pid); // <-- check this again (Nurit)
...
rule sushiGivenInHarvestEqualsPendingSushi(uint256 pid, address user, address to) {
	...
	require sushiToken == SUMMIT();

	uint256 userSushiBalance = sushiToken.balanceOf(e, user);
	uint256 userPendingSushi = pendingSummit(e, pid, user);
...
function callFunctionWithParams(method f, uint256 pid, address sender, address to) {
	...
	} else if (f.selector == pendingSummit(uint256, address).selector) {
		pendingSummit(e, pid, to);
